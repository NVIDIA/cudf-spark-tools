...
- R11. With `HEAP`, reduce `spark.executor.memory` by the full delta and leave the calculated overhead unchanged. Compute the transfer floor independently of any enforced or preserved source-heap baseline as `max(unconstrained calculated heap, HEAP_PER_CORE.min * executor cores)`, and do not reduce heap below that floor. This allows slack in a larger preserved heap to fund PySpark without letting the heuristic undo the normal heap calculation.
...
- KTD6. Compute the heap transfer floor without enforced or preserved source-heap baselines: use the greater of the unconstrained calculated heap and `HEAP_PER_CORE.min * executor cores`. The base layout may use a larger preserved heap; only the slack above this independent floor is transferable. An enforced heap remains immovable. Governs R11.
...
|---|---|---|
| Focused parser and heuristic suites | `cd core && mvn test -Dsuites=com.nvidia.spark.rapids.tool.profiling.ApplicationInfoSuite,com.nvidia.spark.rapids.tool.profiling.OomDetectionSuite,com.nvidia.spark.rapids.tool.tuning.BaseAutoTunerSuite,com.nvidia.spark.rapids.tool.tuning.ProfilingAutoTunerSuiteV2,com.nvidia.spark.rapids.tool.tuning.QualificationAutoTunerSuite` | U1-U6 |
| Full core suite | `cd core && mvn test` | Whole change |
| Multi-version focused tests | `cd core && for buildver in 320 330 340 357; do mvn -Dbuildver="$buildver" test -Dsuites=com.nvidia.spark.rapids.tool.profiling.ApplicationInfoSuite,com.nvidia.spark.rapids.tool.profiling.OomDetectionSuite,com.nvidia.spark.rapids.tool.tuning.BaseAutoTunerSuite,com.nvidia.spark.rapids.tool.tuning.ProfilingAutoTunerSuiteV2,com.nvidia.spark.rapids.tool.tuning.QualificationAutoTunerSuite || exit 1; done` | Listener and heuristic behavior on the oldest profile of each supported Spark minor plus default 3.5.7 |
