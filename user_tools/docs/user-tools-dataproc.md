# NVIDIA cuDF plugin for Apache Spark User Tools on Dataproc

This is a guide for the tools for the NVIDIA cuDF plugin for Apache Spark on [Google Cloud Dataproc](https://cloud.google.com/dataproc).
At the end of this guide, the user will be able to run the cuDF plugin tools to analyze the clusters and
...
User tools variables use the `RAPIDS_USER_TOOLS_*` naming pattern:
  - `RAPIDS_USER_TOOLS_CACHE_FOLDER`: specifies the location of a local directory that the cuDF plugin CLI uses to
  ...
  - `RAPIDS_USER_TOOLS_OUTPUT_DIRECTORY`: specifies the location of a local directory that the cuDF plugin CLI uses to
...
| **tools_jar**                  | Path to a bundled jar including cuDF plugin tool. The path is a local filesystem, or remote gs url                                                                                                                                                                                                                                                                                                             | Downloads the latest `rapids-4-spark-tools_*.jar` from mvn repo                                                                                                                                                                     |    N     |
...
5. The following script runs qualification by passing gs remote directory to store the output:
   ...
   the actual folder of the cuDF plugin Qualification tool. The directory will be mirrored to gs path (`REMOTE_FOLDER`).
...
| Option               | Description                                                                                                                                                                                                                                               | Default                                                                                                                                                                                                                             | Required |
|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| **gpu_cluster**      | The Dataproc-cluster on which the Spark applications were executed. The argument can be a Dataproc-cluster or a valid path to the cluster's properties file (json format) generated by the gcloud-CLI                                                     | None                                                                                                                                                                                                                             | N        |
...
the actual folder of the cuDF plugin Profiling tool. The directory will be mirrored to gs folder if the
