...
class TestS3EndpointResolution:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("AWS_PROFILE", "primary"),
            ("AWS_DEFAULT_PROFILE", "secondary"),
        ],
    )
    def test_s3_endpoint_does_not_fall_back_from_selected_profile(
            self, monkeypatch, tmp_path, selected_env, selected_profile):
        config_file = tmp_path / "aws_config"
        config_file.write_text(
            "[default]\n"
            "endpoint_url = https://default.example\n"
            "[profile fallback]\n"
            "endpoint_url = https://fallback.example\n"
            "[profile primary]\n"
            "region = us-east-1\n"
            "[profile secondary]\n"
            "region = us-west-2\n",
        ...
        monkeypatch.setenv("AWS_CONFIG_FILE", str(config_file))
        monkeypatch.setenv(selected_env, selected_profile)
        if selected_env == "AWS_PROFILE":
